...
module.exports.run = function(event, context, cb) {
  ...
  if (!emailRegex.test(event.email)) {
    return cb(new Error('Please provide a valid email address.'));
  }
  ...
  var sendVerificationEmail = function(data) {
    ...
    var params = {
      FunctionName: 'paperplane-mailer-dev',
    ...
    lambda.invokePromised(params)
      ...
      .then(function(data) {
        return cb(null, {message: 'verification email sent'});
      });
  };
  ...
  var sendDataEmail = function(email) {
    ...
    lambda.invoke(params)
      .then(function(data) {
        return cb(null, {message: 'data email sent'});
      });
  };
  ...
  var handleData = function(data) {
    if (data && 'Item' in data) {
      if (data.Item.isVerified.BOOL) {
        ...
        cb(new Error('Maximum number of verifications exceeded'));
      }
    } else { // create Item
